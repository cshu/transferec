...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    () async {
      ...
      if (null == certnkey) {
        print("cert gen failed"); //undone
        return;
      }
      int len =
          ByteData.sublistView(certnkey, 0, 4).getUint32(0, Endian.little);
      var serverContext = SecurityContext();
      serverContext.useCertificateChainBytes(
          Uint8List.sublistView(certnkey, 4, 4 + len));
      serverContext.usePrivateKeyBytes(
        Uint8List.sublistView(certnkey, 4 + len),
      );
      //
...
void defOnData(HttpRequest req) async {
  ...
  //}
  //++numOfReq;
  //undone if too many requests within a period of time, lock the app
  switch (req.method) {
    case 'GET':
      //pathSegments
      ...
      switch (req.uri.path) {
        case '/':
          ...
          req.response.write(
              '<!DOCTYPE html><html><head><meta charset="utf-8" /><title>Test me</title></head><body>Test body');
          break;
        default:
    ...
    case 'POST':
      try {
        ...
        //NOTE when err is caught here it means txn is already rolled back
        //writeErrorToResp(req.response);
        print('Caught error: $err'); //undone should write to log
        print(stackTrace);
      }
      break;
    default:
